...
export class Engine {
    ...
    getOpenOrders(market, userId) {
        return this.mustGetOrderbook(market).getOpenOrders(userId);
    }
    applyFills(takerUserId, takerSide, baseAsset, quoteAsset, fills) {
    ...
    releaseUnusedBuyFunds(userId, quoteAsset, orderPrice, orderQty, executedQty, fills) {
        const user = this.getOrCreateUserBalance(userId);
        const quote = this.getOrCreateAssetBalance(user, quoteAsset);
    ...
    checkAndLockFunds(userId, baseAsset, quoteAsset, side, price, quantity) {
        const user = this.getOrCreateUserBalance(userId);
        if (side === "buy") {
    ...
    getOrCreateAssetBalance(user, asset) {
        if (!user[asset]) {
