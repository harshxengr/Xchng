...
async function runTest() {
    ...
    await client.query(`
        INSERT INTO "Balance" ("id", "userId", "asset", "available", "locked") 
        VALUES ($1, '2', 'TATA', '1000', '0') ON CONFLICT ("userId", "asset") DO UPDATE SET available = '1000'
    `, [randomUUID()]);
    ...
    const tradeResult = await client.query('SELECT * FROM "Trade" WHERE market = $1 ORDER BY timestamp DESC LIMIT 1', ['TATA_INR']);
    ...
    const user1Btc = await client.query('SELECT available FROM "Balance" WHERE "userId" = $1 AND asset = $2', ['1', 'TATA']);
